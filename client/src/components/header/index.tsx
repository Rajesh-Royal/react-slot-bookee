...
const Header = ({activeTab, setActiveTab}: IHeaderProps) => {
  return (
      <div className="nav-tabs"  onClick={(e) => {
        ...
        <p className={`cursor-pointer ${activeTab === navBarItems.MY_SHIFTS ? "active" : ""}`} title="my">My shifts</p>
        <p className={`cursor-pointer ${activeTab === navBarItems.AVAILABLE_SHIFTS ? "active" : ""}`} title="available">Available shifts</p>
    </div>
  )
}
